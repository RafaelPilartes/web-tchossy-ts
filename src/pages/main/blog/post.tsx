...
function Post() {
  ...
  const idPath = param.postId
  const titlePath = param.postName
  ...
  const listarCategoryPost = docCategory.map((item, index) => {
    return (
      <li key={index}>
        <a href={`${routsNameString.blog.category as string}${item.name}`}>
  ...
  const listarTags = docTags.map((item, index) => {
    return (
      <li key={index}>
        <a href={`${routsNameString.blog.tag as string}${item.name}`}>
          <SLink>{item.name}</SLink>
        </a>
      </li>
    )
          ...
          <div className="post">
            <div className="containerImg">
              ...
              <div className="containerOtherInfo">
                ...
                <a href={`${routsNameString.blog.details as string}${'1234'}`}>
                  <strong>
